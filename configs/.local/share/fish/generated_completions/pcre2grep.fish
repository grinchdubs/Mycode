...
complete -c pcre2grep -s A -l after-context --description 'Output number lines of context after each matching line.'
complete -c pcre2grep -s a -l text --description 'Treat binary files as text.  This is equivalent to --binary-files=text.'
complete -c pcre2grep -s B -l before-context --description 'Output number lines of context before each matching line.'
complete -c pcre2grep -l binary-files --description 'Specify how binary files are to be processed.'
complete -c pcre2grep -l buffer-size --description 'Set the parameter that controls how much memory is used for buffering files t…'
...
complete -c pcre2grep -l line-buffered --description 'When this option is given, input is read and processed line by line, and the …'
...
complete -c pcre2grep -l match-limit --description 'Processing some regular expression patterns can require a very large amount o…'
...
complete -c pcre2grep -s w -l word-regex -l word-regexp --description 'Force the patterns to match only whole words.'
complete -c pcre2grep -s x --description 'given any number of times.  If a directory matches both --include-dir and.'
complete -c pcre2grep -l line-regex -l line-regexp --description 'Force the patterns to be anchored (each must start matching at the beginning …'
complete -c pcre2grep -l xxx-regexp -l xxx-regex --description '(PCRE2 terminology).  However, the --file-list, --file-offsets,.'
