...
complete -c hdiutil -o nobrowse --description 'render any volumes invisible in applications such as the OS X Finder.'
...
complete -c hdiutil -o autoopenrw --description 'do [not] auto-open read/write volumes.'
...
complete -c hdiutil -o fs --description 'br where filesystem is one of HFS+, HFS+J (JHFS+), HFSX, JHFS+X, FAT32, ExFAT…'
...
complete -c hdiutil -o fsargs --description 'br additional arguments to pass to whatever newfs program is implied by "fs".'
...
complete -c hdiutil -o only-joliet --description 'A glob expression of objects that should only be exposed in Joliet.'
...
complete -c hdiutil -o sleepallowed --description 'allow machine to sleep while compacting.  The default is not allowed.'
